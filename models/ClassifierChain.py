...
def cc_fit(X, Y, alpha):
    """
    Fit a classifier chain based on logistic regression.
    ...
    # Fit model
    model = ClassifierChain(LogisticRegression(C=C, max_iter=10000))
    t0 = time.time()
    model.fit(X, Y)
    t1 = time.time()
...
def cc_cv(X, Y, folds, grid, metric):
    ...
    for grid_index in range(grid.shape[0]):
        ...
        for i in range(len(folds)):
            ...
            model = cc_fit(X[train, :], Y[train, :], alpha)
