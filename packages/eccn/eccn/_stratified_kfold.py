...
def _find_candidate_folds(Y, k, imbalance_order):
    ...
    for l in range(L):
        # Try 5 times to find folds that use the specified columns of Y for
        # stratifying
        for i in range(5):
            try:
                folds = _generate_kfold(Y, k, imbalance_order[: (L - l)])

                # If a valid set of folds is found, immediately return
                return folds
            except ValueError:
                print("Folds were not generated")
...
def multilabel_kfold(Y, k):
    ...
    # Try five times to find valid folds that have label heterogeneity
    for i in range(5):
